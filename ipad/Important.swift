...
class Important: UIViewController,UINavigationBarDelegate,UINavigationControllerDelegate, GIDSignInUIDelegate, GIDSignInDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func CheckSpeak(){
        ...
        if self.speaker.synth.isSpeaking {
            ...
            timer = Timer.scheduledTimer(timeInterval: 5.0, target: self, selector: #selector(Important.update), userInfo: nil, repeats: true)
        }
    }
    ...
    func update() {
        ...
        if petitions.count == 0 {
            ...
            if self.speaker.synth.isSpeaking {
                ...
                timerSpeak = Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(Important.CheckSpeak), userInfo: nil, repeats: true)
                self.refreshControl.endRefreshing()
            }else{
