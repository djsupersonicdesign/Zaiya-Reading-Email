...
//  ipad
//
//  Created by domingo jesus bagalacsa on 05/01/2017.
//  Copyright © 2017 SuperSonicDesign.com. All rights reserved.
//
...
class Spam: UIViewController,UINavigationBarDelegate,UINavigationControllerDelegate, GIDSignInUIDelegate, GIDSignInDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func CheckSpeak(){
        ...
        if self.speaker.synth.isSpeaking {
            ...
            timer = Timer.scheduledTimer(timeInterval: 5.0, target: self, selector: #selector(Spam.update), userInfo: nil, repeats: true)
        }
    }
    ...
    func update() {
        ...
        if petitions.count == 0 {
            ...
            if self.speaker.synth.isSpeaking {
                ...
                timerSpeak = Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(Spam.CheckSpeak), userInfo: nil, repeats: true)
                self.refreshControl.endRefreshing()
            }else{
